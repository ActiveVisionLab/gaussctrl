...
class GaussEditPipeline(VanillaPipeline):
    ...
    def __init__(
        ...
        view_num = self.config.datamanager.sampled_view_num_every * self.config.datamanager.sampled_view_split # len(self.datamanager.train_dataset._dataparser_outputs.image_filenames) 
        anchors = list(range(0, view_num, view_num // 4)) + [view_num]
                
        self.ref_indices = [np.random.randint(anchor, anchors[idx+1]) for idx, anchor in enumerate(anchors[:-1])]  # [18, 23]
